...
enum class EntitySound{ None = -1, Footstep, Attack, Hurt, Death, COUNT };
...
public:
	...
	const std::string& GetSound(const EntitySound& l_snd) const {
		...
		return((size_t)l_snd < static_cast<size_t>(EntitySound::COUNT) ? m_params[(int)l_snd].m_sound : empty);
	}

	bool IsSoundFrame(const EntitySound& l_snd, int l_frame) const {
		if ((size_t)l_snd >= static_cast<size_t>(EntitySound::COUNT)) { return false; }
