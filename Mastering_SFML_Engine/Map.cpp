...
Map::Map(Window* l_window, EntityManager* l_entityManager, TextureManager* l_textureManager)
	...
	m_entityManager(l_entityManager), m_tileMap({ 0, 0 }, m_tileSet), m_playerId(-1), 
	m_tileSet(l_textureManager), m_gameTime(0.f), m_dayLength(30.f) {
	...
	for (auto x = l_from.x; x <= l_to.x; ++x) {
		for (auto y = l_from.y; y <= l_to.y; ++y) {
			for (auto layer = l_from.z; layer <= l_to.z; ++layer) {
				...
				sprite.setPosition(static_cast<float>(x * Sheet::Tile_Size), 
...
void Map::Update(float l_dT) {
	...
	if (!shader) { return; }
	if (m_stateManager->HasState(StateType::MapEditor)) { timeNormal = 0.1f; }
