...
void S_Sound::Update(float l_dT) {
	...
	for (auto &entity : m_entities) {
		auto c_pos = entities->GetComponent<C_Position>(entity, Component::Position);
		auto position = c_pos->GetPosition();
		auto elevation = c_pos->GetElevation();

		auto IsListener = entities->HasComponent(entity, Component::SoundListener);
		...
		auto c_snd = entities->GetComponent<C_SoundEmitter>(entity, Component::SoundEmitter);
...
void S_Sound::Notify(const Message& l_message) {
	...
	EntityManager* entities = m_systemManager->GetEntityManager();
	auto IsListener = entities->HasComponent(l_message.m_receiver, Component::SoundListener);
	...
	case EntityMessage::Frame_Change:
		...
		auto state = entities->GetComponent<C_State>(l_message.m_receiver, Component::State)->GetState();
...
sf::Vector3f S_Sound::MakeSoundPosition(const sf::Vector2f& l_entityPos, unsigned int l_elevation) {
	return sf::Vector3f(l_entityPos.x, static_cast<float>(l_elevation * Sheet::Tile_Size), l_entityPos.y);
}
...
void S_Sound::EmitSound(const EntityId& l_entity, const EntitySound& l_sound, bool l_useId, bool l_relative, int l_checkFrame)
{
	...
	// Is a sound emitter.
	EntityManager* entities = m_systemManager->GetEntityManager();
	auto c_snd = entities->GetComponent<C_SoundEmitter>(l_entity, Component::SoundEmitter);
	...
	auto c_pos = entities->GetComponent<C_Position>(l_entity, Component::Position);
	auto pos = (l_relative ? sf::Vector3f(0.f, 0.f, 0.f)
